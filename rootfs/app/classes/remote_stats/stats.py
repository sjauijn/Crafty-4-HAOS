...
class Stats:
    ...
    def _all_disk_usage() -> t.List[DiskDataDict]:
        ...
        return Stats._drop_ancestor_mounts(disk_data)

    @staticmethod
    def _drop_ancestor_mounts(
        disk_data: t.List[DiskDataDict],
    ) -> t.List[DiskDataDict]:
        mounts = [entry["mount"] for entry in disk_data]
        filtered = []
        for entry in disk_data:
            mount = entry["mount"]
            normalized = mount.rstrip("/") + "/"
            is_ancestor = any(
                other != mount and other.rstrip("/").startswith(normalized)
                for other in mounts
            )
            if not is_ancestor:
